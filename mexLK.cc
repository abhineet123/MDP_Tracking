...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	cv::Mat _ptsI = cv::Mat(nPts, 2, CV_32FC1);
	cv::Mat _ptsJ = cv::Mat(nPts, 2, CV_32FC1);
	...
	for(int i = 0; i < nPts; i++)
	{
		...
		_ptsI.at<float>(i, 0) = ptsI[2 * i];
		_ptsI.at<float>(i, 1) = ptsI[2 * i + 1];

		_ptsJ.at<float>(i, 0) = ptsJ[2 * i];
		_ptsJ.at<float>(i, 1) = ptsJ[2 * i + 1];
	...
	printf("Level: %d\n", Level);
	cv::Mat img_1(IMG[I]), img_2(IMG[J]);
	mtf::utils::drawPts<float>(img_1, _ptsI, cv::Scalar(0, 0, 0), 2);
	mtf::utils::drawPts<float>(img_2, _ptsJ, cv::Scalar(0, 0, 0), 2);
	...
	mtf::utils::printMatrixToFile<float>(_ptsI, nullptr, "ptsI.txt", "%.4f");
	mtf::utils::printMatrixToFile<float>(_ptsJ, nullptr, "ptsJ.txt", "%.4f");
