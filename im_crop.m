...
% if the top left corner of the bounding box is greater than one then the top left corner of the old image from where the batch was extracted will be same as top left corner of the bounding box so subtracting it will just make it zero and then the add one so that's perfectly fine and yeah in that case the bottom right also make sense;
% but if this top left corner of the given bounding box is less than one then asked one would be one and subtracting a negative number for instance if this top left corner of BB is negative and subtracting a negative number makes it the positive number that then we are moving it away from the top left corner off the new image in which case you the whole batch the location of this extracted batch within the new image will be shifted by the same amount as the negative magnitude of top left corner of the given bounding box;
% it is not quite clear how such a thing is going to work but I imagine that Matt left can dynamically resize matrices so this will probably not really create an error it will just extend the original image matrix which has the same size as the bounding box to be larger than the size so that dispatch pixels can be placed at these shifted locations;
w = bb_width(bb);
h = bb_height(bb);
I = uint8(zeros(h, w));
